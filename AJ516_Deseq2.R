...
library(pheatmap)
library(dplyr)
setwd("C:/Users/john.connell/Documents/Bioinformatics_2022/projects/Fusarium_oxysporum/deseq2/AJ516")

#Create tx2gene table 

gene.t <- paste0("g", 1:24988, ".t", 1)
gene.t <- as.data.frame(gene.t)
gene <- paste0("g", 1:24988)
...
condition <- function(designtable){
  if ((designtable == "AJ516_1M") || (designtable == "AJ516_2M") || (designtable == "AJ516_3M")|| (designtable == "AJ516_4M"))
    return ("control")
  else return ("experimental")
}
designtable$condition <- lapply(X = designtable$designtable, FUN = condition)
designtable$ID <- sapply(strsplit(designtable$designtable, split = "AJ516_"),'[',2)
colnames(designtable) <- c("Sample", "Condition", "ID")
...
summary(sig.res)
write.table(sig.res,"C:/Users/john.connell/Documents/Bioinformatics_2022/projects/Fusarium_oxysporum/deseq2/AJ516/all_data.txt",sep="\t",na="",quote=F)
...
coord_fixed()
pca_plot
ggsave("C:/Users/john.connell/Documents/Bioinformatics_2022/projects/Fusarium_oxysporum/deseq2/AJ516/PCA_vst_false.jpeg", pca_plot, dpi=300, height=10, width=12)

#Plot heatmap of differential expression 

all_data <- read.table("C:/Users/john.connell/Documents/Bioinformatics_2022/projects/Fusarium_oxysporum/deseq2/AJ516/all_data.txt")
subset <- all_data[2]

highlfc <- filter(subset, log2FoldChange >= 10 | log2FoldChange <= -10)
sortedhlfc <- highlfc[order(highlfc$log2FoldChange), , drop = FALSE]
heatmapH <- pheatmap(sortedhlfc, cluster_rows=FALSE, cluster_cols=FALSE, color=colorRampPalette(c("darkblue", "white", "red"))(50), main = "AJ516 l2FC") 
...
save_pheatmap_pdf(heatmapH, "C:/Users/john.connell/Documents/Bioinformatics_2022/projects/Fusarium_oxysporum/deseq2/AJ516/AJ516_l2FC.pdf")
